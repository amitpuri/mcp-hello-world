"""
Model API clients for Ollama, Claude, and OpenAI.
"""
        ...
        try:
            response = await self.http_client.post(
                ...
                json={
                    ...
                    "stream": False,
                    "options": {
                        "temperature": options.get("temperature", DEFAULT_CONFIGS[ModelProvider.OLLAMA]["temperature"]),
                        "num_predict": options.get("max_tokens", DEFAULT_CONFIGS[ModelProvider.OLLAMA]["max_tokens"]),
                    }
                }
            )
        ...
        try:
            ...
            response = await loop.run_in_executor(
                ...
                lambda: self.anthropic_client.messages.create(
                    model=model_name,
                    max_tokens=options.get("max_tokens", DEFAULT_CONFIGS[ModelProvider.CLAUDE]["max_tokens"]),
                    temperature=options.get("temperature", DEFAULT_CONFIGS[ModelProvider.CLAUDE]["temperature"]),
        ...
        try:
            ...
            response = await loop.run_in_executor(
                ...
                lambda: self.openai_client.chat.completions.create(
                    model=model_name,
                    messages=[{"role": "user", "content": prompt}],
                    max_tokens=options.get("max_tokens", DEFAULT_CONFIGS[ModelProvider.OPENAI]["max_tokens"]),
                    temperature=options.get("temperature", DEFAULT_CONFIGS[ModelProvider.OPENAI]["temperature"])
                )
            )
...
async def call_model(provider: str, prompt: str, model: Optional[str] = None, options: Optional[Dict] = None) -> Dict:
    """Convenience function to call any model."""
    clients = get_model_clients()
    return await clients.call_model(provider, prompt, model, options)

async def get_available_models(provider: str) -> Dict:
    """Convenience function to get available models."""
    clients = get_model_clients()
    return await clients.get_available_models(provider)